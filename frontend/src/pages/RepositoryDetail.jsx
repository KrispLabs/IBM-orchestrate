    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">{repo?.repo_name}</h1>
            <a
              href={repo?.repo_url}
              target="_blank"
          ...
          {timeline && timeline.length > 0 ? (
            <div className="space-y-4">
              {timeline.map((event, index) => (
                <div
                  key={index}
                  className="flex items-start space-x-4 p-4 bg-slate-900 rounded-lg border border-slate-700"
                >
        ...
        {/* Test Files */}
        <div className="card">
          <h3 className="text-lg font-semibold text-white mb-4">Generated Test Files</h3>
          {testFiles && testFiles.length > 0 ? (
            <div className="space-y-3">
              {testFiles.map((test) => (
                <Link
                  key={test.id}
                  to={`/test/${test.id}`}
                  className="block p-4 bg-slate-900 rounded-lg border border-slate-700 hover:border-slate-600 transition-colors"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <FileCode className="text-blue-500" size={20} />
                      <div>
                        <p className="text-white font-medium">{test.code_file?.file_path}</p>
            ...
            <div className="text-center py-8">
              <FileCode className="mx-auto text-slate-600" size={48} />
              <p className="text-slate-400 mt-4">No test files generated yet</p>
